...
and extended by [Bunea et al. (2011)](#3) and [Abram et al.](#1). These methods
...
(VIP) algorithm (Bach, 2008; Bunea et al., 2011), and the bootstrap
distribution quantile (QNT) algorithm (Abram et al., 2016). Beyond implementing
...
VIP method ([Bach](#2) and [Bunea et al.](#3)), which selects all variables
that were selected (non-zero coefficient) in >= 95% of the bootstrapped models.
We'll use the regularization parameter lambda that minimizes
...
variables, we can use the `plot_selected_variables` which will give us pretty
...
To address this, we can activate the `fast = TRUE` flag in bolasso. Instead of
...
<a id="1">[3]</a>Abram, Samantha V et al. “Bootstrap Enhanced Penalized
...
<a id="2">[1]</a>Bach, Francis. “Bolasso: Model Consistent Lasso Estimation 
through the Bootstrap.” ArXiv:0804.1302 [Cs, Math, Stat], 2008. 
https://arxiv.org/abs/0804.1302.

<a id="3">[2]</a>Bunea, Florentina et al. “Penalized least squares regression
